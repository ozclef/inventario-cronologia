...
armé una versión mejorada y funcional: genera las columnas de semanas sin `document.write`, permite **agregar actividades**, **cargar** un `db.json` remoto (o local), **exportar** JSON descargable y **llenar** el formulario desde un JSON con la estructura anidada que usaste (nombres con puntos). También añadí manejo de la tabla de 15 semanas como un array por actividad.

Copia-pega todo esto en tu `index.html` (reemplaza tu `body` actual). Funciona si `db.json` está en la misma carpeta o si usas la URL cruda tipo `https://raw.githubusercontent.com/ozclef/inventario-cronologia/main/db.json` (ojo: GitHub raw puede dar problemas de CORS en algunos escenarios — te explico abajo cómo evitarlo):
...
<head>
  ...
  <title>Formato: Seguimiento de Estancia/Estadía</title>
  <style>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildHeader(){
    ...
    tr1.innerHTML = '<th>#</th><th class="left">Descripción</th><th colspan="'+WEEKS+'">Semanas 1–'+WEEKS+'</th>';
  ...
  function renderActividades(){
    ...
    actividades.forEach((act, idx) => {
      ...
      // añadimos una celda extra para el botón (oculto en tamaños pequeños)
  ...
  function formToObject(form){
    ...
    // añadir actividades desde estado
  ...
  // Botón Imprimir (usa print)
  btnImprimir.addEventListener('click', () => window.print());

  // Botón Cargar: pedir URL o usar local db.json
  btnCargar.addEventListener('click', async () => {
    const url = prompt('Introduce la URL RAW del JSON o deja vacío para usar "db.json" local:', '');
  ...
  // inicializar encabezado y tabla vacía
  buildHeader();
  renderActividades();
});
</script>
...
### Notas importantes / consejos rápidos

* **Nombre del archivo JSON:** `db.json` está bien. Si usas `fetch()` con la URL raw de GitHub, usa la forma `https://raw.githubusercontent.com/ozclef/inventario-cronologia/main/db.json`.

  * Ejemplo en el prompt de carga: pega `https://raw.githubusercontent.com/ozclef/inventario-cronologia/main/db.json`.
* **CORS:** GitHub raw suele servir con CORS permitido, pero a veces puede fallar según cómo esté publicado el sitio. Si te da error al cargar desde la URL cruda, descarga el JSON a tu repo (mismo directorio que `index.html`) y deja `db.json` local: así `fetch('db.json')` funcionará.
* **Estructura del JSON sugerida:** (esto es lo que importará/creará el script)

```json
{
  "programaAcademico": "Ingeniería X",
...
* Si quieres que yo **genere un JSON de ejemplo** relleno con datos que justifiquen “500 horas y 4 meses” (para la entrega de la universidad), dímelo y lo creo ya mismo con la distribución por semanas/horas para que lo descargues. Puedo incluir una bitácora por actividad y sumar horas totales por semana para llegar a 500 horas en 4 meses (4 meses ≈ 16–18 semanas; tú usaste 15 semanas, puedo ajustar).

¿Quieres que haga un `db.json` de ejemplo que muestre 500 horas distribuidas (por ejemplo 15 semanas -> ~33 horas/semana = 495, o 16 semanas -> 31.25)? Pongo los números y la bitácora con tareas reales para que la universidad lo vea *chido* 😎.
